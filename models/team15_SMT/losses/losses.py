...
class EdgeLoss(nn.Module):
    """
    边缘保持损失 - 专注于保持和增强图像边缘
    用于改善细节丰富图像(如建筑物和毛发)的边缘清晰度
    """
    def __init__(self, loss_weight=1.0, edge_threshold=0.05):
        super(EdgeLoss, self).__init__()
        print('********Edge loss is used.')
        self.loss_weight = loss_weight
    ...
    def forward(self, sr, hr):
        ...
        # 提取HR图像的边缘
        hr_edges = self._extract_edges(hr, sobel_x, sobel_y)
        # 提取SR图像的边缘
        sr_edges = self._extract_edges(sr, sobel_x, sobel_y)
        
        # 计算边缘强度差异
        edge_diff = torch.abs(sr_edges - hr_edges)
        
        # 创建边缘掩码 - 识别HR中的重要边缘区域
        edge_mask = (hr_edges > self.edge_threshold).float()
        
        # 选择性加权 - 对重要边缘区域施加更高权重
        weighted_diff = edge_diff * (1 + (self.alpha - 1) * edge_mask)
    ...
    def _extract_edges(self, img, sobel_x, sobel_y):
        """
        使用Sobel算子提取图像边缘
        
        Args:
            img (Tensor): 输入图像 [B, C, H, W]
            sobel_x (Tensor): X方向Sobel算子
            sobel_y (Tensor): Y方向Sobel算子
        """
        # 转为灰度图
        if img.size(1) == 3:  # 彩色图像
    ...
    def _compute_gradient_orientation(self, edge_x, edge_y):
        """
        计算梯度方向
        用于方向敏感的边缘比较(可选功能)
        """
        # 计算梯度方向(弧度)
        orientation = torch.atan2(edge_y, edge_x)
        # 转到[0, π]区间
