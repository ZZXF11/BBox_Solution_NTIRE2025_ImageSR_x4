...
from models.team15_SMT.models.sr_model import SRModel, SRModel_debug
...
@MODEL_REGISTRY.register()
class MambaIRv2Model_debug(SRModel_debug):
    """MambaIRv2 model for image restoration."""

    # test by partitioning
    def test(self):
        _, C, h, w = self.lq.size()
        split_token_h = h // 1000 + 1  # number of horizontal cut sections
        split_token_w = w // 1000 + 1  # number of vertical cut sections
        # import pdb; pdb.set_trace()
        # padding
        mod_pad_h, mod_pad_w = 0, 0
        if h % split_token_h != 0:
