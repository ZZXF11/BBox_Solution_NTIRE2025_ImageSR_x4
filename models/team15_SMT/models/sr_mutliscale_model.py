...
from .base_multiscale_model import BaseModel
import json
import os
...
class SRModel_mutliscale(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRModel_mutliscale, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, self.output_x2, self.output_x3 = self.net_g(self.lq)

        # 创建2倍和3倍分辨率的GT
        _, _, h_gt, w_gt = self.gt.shape
        gt_x3 = F.interpolate(self.gt, size=(h_gt//4*3, w_gt//4*3), mode='bicubic', align_corners=False)
        gt_x2 = F.interpolate(self.gt, size=(h_gt//4*2, w_gt//4*2), mode='bicubic', align_corners=False)
        ...
        if self.cri_pix:
            ...
            l_pix_x3 = self.cri_pix(self.output_x3, gt_x3)
            l_pix_x2 = self.cri_pix(self.output_x2, gt_x2)
            l_total += (l_pix + l_pix_x3*0.5 + l_pix_x2*0.5) / 2
