...
from models.team15_SMT.archs.arch_util import to_2tuple, trunc_normal_
from mamba_ssm.ops.selective_scan_interface import selective_scan_fn, selective_scan_ref
from models.team15_SMT.utils.registry import ARCH_REGISTRY
...
class MambaIRv2_Multiscale(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 in_chans=3,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample3 = Upsample(3, num_feat)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x3 = self.conv_last(self.upsample3(x))
            x2 = self.conv_last(self.upsample2(x))
            x = self.conv_last(self.upsample4(x))
        elif self.upsampler == 'pixelshuffledirect':
            # for lightweight SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, params)) + x
        ...
        x3 = x3 / self.img_range + self.mean

        # unpadding
        x = x[..., :h_ori * self.upscale, :w_ori * self.upscale]
        x2 = x2[..., :h_ori * 2, :w_ori * 2]
        x3 = x3[..., :h_ori * 3, :w_ori * 3]
        # import pdb; pdb.set_trace()
        return x, x2, x3
